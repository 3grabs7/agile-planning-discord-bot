...
{
    public abstract class RepportLoggerStepBase : IReportLoggerStep
    {
        protected readonly string _content;

        public RepportLoggerStepBase(string content, Enums.Step step)
        {
            _content = content;
        }

        public Action<DiscordMessage> OnMessageAdded { get; set; } = delegate { };
        public abstract IReportLoggerStep NextStep { get; }
